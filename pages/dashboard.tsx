...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      ...
      if (currentUser) {
        console.log('User already authenticated, fetching data immediately')
        await fetchUserData()
    ...
    // Check for balance updates every 3 seconds (even more frequent for admin deposits)
    ...
    const newsInterval = setInterval(() => {
      updateNews()
    }, 15000) // Update news every 15 seconds
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          // Create minimal user data for other components
  ...
  const updateNews = () => {
    // Rotate news items to show different articles
    setNews(prev => {
      const shuffled = [...allNewsItems].sort(() => Math.random() - 0.5)
      return shuffled.slice(0, 3).map((item, index) => ({
        ...item,
        time: `${(index + 1) * 2} hours ago`,
        // Update time to make it feel more real-time
        summary: item.summary + (Math.random() > 0.5 ? ' Market analysts predict continued growth...' : ' Investors are closely monitoring developments...')
      }))
    })
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-30 w-64 sm:w-72 bg-black/30 backdrop-blur-xl border-r border-white/10 transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full pt-16">
          <nav className="flex-1 px-4 py-6 space-y-2">
            <Link href="/dashboard" className="flex items-center space-x-3 px-4 py-3 bg-purple-500/20 border border-purple-500/50 rounded-lg text-white">
              <Home className="w-5 h-5" />
              <span>Dashboard</span>
            </Link>
                        <button 
              className="relative flex items-center space-x-3 px-4 py-3 text-gray-300 hover:bg-white/10 rounded-lg transition-colors w-full"
              onClick={() => {
                // Trigger chat opening
                const chatButton = document.querySelector('[data-chat-trigger]') as HTMLButtonElement;
                if (chatButton) chatButton.click();
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute top-3 right-3 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center animate-pulse">
                  {unreadCount > 9 ? '9+' : unreadCount}
                </span>
              )}
            </button>
        ...
        <div className="py-4 sm:py-8">
          ...
          >
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 sm:mb-6 space-y-2 sm:space-y-0">
              <h2 className="text-lg sm:text-xl font-bold text-white flex flex-col sm:flex-row sm:items-center">
                <div className="flex items-center">
                  <Newspaper className="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-purple-400" />
                  Investment News
                </div>
                <div className="ml-0 sm:ml-3 flex items-center space-x-1 mt-1 sm:mt-0">
                  <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
            ...
            <div className="space-y-4">
              {news.map((article, index) => (
                <motion.div
                  ...
                  transition={{ delay: 0.1 * index }}
                  className="border-l-2 border-purple-500/30 pl-3 sm:pl-4 hover:border-purple-500/60 transition-all duration-300 hover:bg-white/5 rounded-r-lg p-2 sm:p-3 group"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1">
                      <div className="flex items-center space-x-1 sm:space-x-2 mb-1">
                        <h3 className="text-white font-semibold text-xs sm:text-sm hover:text-purple-400 transition-colors cursor-pointer group-hover:scale-105 transform transition-transform line-clamp-2">
                          {article.title}
                        </h3>
                        <span className={`px-1.5 sm:px-2 py-1 rounded-full text-xs font-medium flex-shrink-0 ${
                      ...
                      <div className="flex flex-wrap items-center gap-1 sm:gap-2 mb-2">
                        <span className="text-purple-400 text-xs font-medium">{article.source}</span>
                        ...
                        <span className={`px-1.5 sm:px-2 py-0.5 rounded text-xs ${
                          ...
                          article.impact === 'medium' ? 'bg-yellow-500/20 text-yellow-400' :
